...
from app.my_graph.tools import analyze_russian_grammar_impl, generate_flashcards_from_analysis_impl
...
class BulkTextProcessor:
    ...
    def start_bulk_processing(self, text: str, user_id: int) -> str:
        ...
        logger.info(f"Started bulk processing job {job_id} for user {user_id} with {len(russian_words)} words")
    ...
    async def _process_job_async(self, job: BulkProcessingJob, words: List[str]):
        ...
        try:
                    ...
                    try:
                        ...
                        if analysis_result.get("success"):
                            ...
                            if flashcard_result.get("success"):
                                cards_generated = flashcard_result.get("flashcards_generated", 0)
                                total_flashcards += cards_generated
                                
                                # Track word types
                                word_type = flashcard_result.get("word_type")
                                if word_type:
                                    job.processed_word_types[word_type] = job.processed_word_types.get(word_type, 0) + 1
                                
                                logger.info(f"Job {job.job_id}: Generated {cards_generated} flashcards for word '{word}'")
                            else:
                                logger.warning(f"Job {job.job_id}: Failed to generate flashcards for word '{word}': {flashcard_result.get('error')}")
                                job.failed_words.append({"word": word, "error": "flashcard_generation_failed"})
                        else:
                            logger.warning(f"Job {job.job_id}: Failed to analyze word '{word}': {analysis_result.get('error')}")
                            job.failed_words.append({"word": word, "error": "analysis_failed"})
                    
                    except Exception as e:
                        logger.error(f"Job {job.job_id}: Error processing word '{word}': {e}")
            ...
            logger.info(f"Completed bulk processing job {job.job_id}: {total_flashcards} flashcards generated from {job.processed_words} words")
