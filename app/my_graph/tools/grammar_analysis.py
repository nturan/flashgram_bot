...
def analyze_russian_grammar_impl(russian_word: str) -> Dict[str, Any]:
    ...
    try:
        logger.info(f"Analyzing Russian word '{russian_word}' with PyMorphy2")
        
        # Use pymorphy2 for morphological analysis
        result = pymorphy2_analyze_russian_grammar_impl(russian_word)
        
        if result.get("success"):
            logger.info(f"PyMorphy2 analysis successful for '{russian_word}'")
            return {"word": russian_word, "analysis": result, "success": True}
        else:
            logger.warning(f"PyMorphy2 analysis failed for '{russian_word}': {result.get('message', 'Unknown error')}")
            return {"word": russian_word, "analysis": result, "success": False}

    except Exception as e:
        logger.error(f"Error in grammar analysis for '{russian_word}': {e}")
