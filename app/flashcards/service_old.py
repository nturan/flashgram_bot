...
class FlashcardService:
    ...
    def format_question_for_bot(
        ...
        try:
            if isinstance(flashcard, TwoSidedCard):
                ...
                question = flashcard.get_question()
                choice_type = (
                    "multiple answers" if flashcard.allow_multiple else "one answer"
                )
                text = f"📝 *Multiple Choice* (select {choice_type})\n\n{question}"

                # Create inline keyboard with options and edit/delete buttons
                keyboard = self._create_multiple_choice_keyboard_with_controls(
                    flashcard
                )
                return text, keyboard
        ...
        try:
            ...
            control_buttons = [
                ...
                InlineKeyboardButton(
                    "🗑️ Delete", callback_data=f"delete_{flashcard.id}"
                ),
            ]
    ...
    def _create_edit_delete_keyboard(self, flashcard):
        ...
        try:
            ...
            buttons = [
                [
                    InlineKeyboardButton(
                        "✏️ Edit Card", callback_data=f"edit_{flashcard.id}"
                    ),
                    InlineKeyboardButton(
                        "🗑️ Delete Card", callback_data=f"delete_{flashcard.id}"
                    ),
                ],
                [
                    InlineKeyboardButton(
                        "📝 Answer", callback_data=f"answer_{flashcard.id}"
                    )
                ],
            ]
    ...
    def check_answer(
        ...
        try:
            if isinstance(flashcard, TwoSidedCard):
                ...
                user_answers = self._parse_fill_in_blank_answer(
                ...
                if is_correct:
                    ...
                    feedback = (
                        f"❌ Incorrect. The correct answers are: {correct_answers}"
                    )
                ...
                selected_indices = self._parse_multiple_choice_answer(
    ...
    def _parse_fill_in_blank_answer(
        self, user_input: str, expected_count: int
    ) -> List[str]:
    ...
    def _parse_multiple_choice_answer(
        self, user_input: str, option_count: int
    ) -> List[int]:
