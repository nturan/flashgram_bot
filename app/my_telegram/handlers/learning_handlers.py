...
async def finish_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if session.learning_mode:
            ...
            accuracy_text = f"🎯 Accuracy: {(score/total*100):.1f}%" if total > 0 else "No questions answered."
...
async def ask_next_question(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if session.flashcards:
            ...
            accuracy_text = f"🎯 Accuracy: {(score/total*100):.1f}%" if total > 0 else ""
    ...
    try:
        ...
        if session.flashcards:
            ...
            try:
                ...
                logger.warning(f"Markdown parsing failed for callback question: {markdown_error}")
                await query.message.reply_text(
                    question_text,
                    reply_markup=keyboard
                )
