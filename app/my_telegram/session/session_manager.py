...
class SessionManager:
    ...
    def start_learning_session(self, user_id: int, flashcards: list) -> UserSession:
        ...
        logger.info(
            f"Started learning session for user {user_id} with {len(flashcards)} flashcards"
        )
        return session
    ...
    def start_editing_session(self, user_id: int, flashcard_id: str) -> UserSession:
        ...
        logger.info(
            f"Started editing session for user {user_id}, flashcard {flashcard_id}"
        )
        return session

    def start_regenerating_session(
        self, user_id: int, flashcard_id: str
    ) -> UserSession:
        ...
        logger.info(
            f"Started regeneration session for user {user_id}, flashcard {flashcard_id}"
        )
        return session
